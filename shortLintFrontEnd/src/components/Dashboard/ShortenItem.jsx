...
const ShortenItem = ({originalUrl, shortUrl, clickCount, createdDate}) => {
  ...
  return (
    <div
      ...
      <div className="flex sm:flex-row flex-col sm:justify-between w-full sm:gap-0 gap-5 py-5">
        ...
        <div className="flex flex-1 sm:justify-end items-center gap-2">
          ...
          <button
            ...
            disabled={isDeleting}
            className="p-2 rounded-md border bg-emerald-300 hover:bg-emerald-200 disabled:opacity-60"
            title="Download/Share QR (PNG)"
          >
            <IoQrCode className="text-[22px]"/>
          </button>
      ...
      {/* Analytics Section */}
      <div
        className={`${analyticToggle ? "flex" : "hidden"} max-h-96 sm:mt-0 mt-5 min-h-96 relative border-t-2 w-full overflow-hidden`}>
        {loader ? (
